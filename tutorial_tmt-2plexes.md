...
These data are part of a kidney clear cell carcinoma dataset from the Clinical Proteomic Tumor Analysis Consortium ([CPTAC](https://proteomics.cancer.gov/)), where patient samples have been labeled with the TMT 10-plex reagent. These data were acquired on an Orbitrap Fusion Lumos and are publicly available from the [CPTAC data portal](https://cptac-data-portal.georgetown.edu/study-summary/S050). It includes both samples that were phosphopeptide-enriched and samples that are unenriched (whole proteome), this tutorial will analyze both, starting with the phosphopeptides.
